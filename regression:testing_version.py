...
def decyrs_to_datetime(decyrs):      # converts date in decimal years to a datetime object (from https://stackoverflow.com/questions/20911015/decimal-years-to-datetime-in-python)
    ...
    for l in range(len(decyrs)):
        ...
        ##print(l, result2)
        output[l] = np.datetime64(result2).astype('datetime64[M]')
    return output
...
timec01 = np.array(data_cleaned[0,:])-1/24     # time in fractional year CE (subtract 1/24 so that time corresponds to start of month (as with sst datasets), rather than mid-month)
print(timec01)
...
try:
    ...
    print("_______________", timec1, "___________________")
